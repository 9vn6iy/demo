...
static void *stats_thread(void *arg) {
  // create a harmless reference to variable "arg"
  // to avoid compiler complain about unused variable
  (void)arg;
...
int main(int argc, char *argv[]) {
  ...
  if (opt_tpacket_v3) {
    ...
    if (setsockopt(psk, SOL_PACKET, PACKET_TX_RING, &ring->req,
                   sizeof(ring->req)) == -1) {
    ...
    if ((ring->map = mmap(NULL, ring->req.tp_block_size * ring->req.tp_block_nr,
                          PROT_READ | PROT_WRITE, MAP_SHARED | MAP_LOCKED, psk,
                          0)) == MAP_FAILED) {
      perror("mmap");
      close(psk);
      exit(EXIT_FAILURE);
  ...
  if (opt_tpacket_v3) {
    ...
    if (bind(psk, (struct sockaddr *)&sa_ll, sizeof(struct sockaddr_ll)) ==
        -1) {
      perror("bind");
      close(psk);
      exit(EXIT_FAILURE);
  ...
  //  50:65:f3:65:c5:9e
  eh->ether_dhost[0] = 0x50;
  eh->ether_dhost[1] = 0x65;
  eh->ether_dhost[2] = 0xf3;
  eh->ether_dhost[3] = 0x65;
  eh->ether_dhost[4] = 0xc5;
  eh->ether_dhost[5] = 0x9e;
  ...
  if (opt_tpacket_v3) {
    for (;;) {
      ...
      for (i = 0; i < ring->req.tp_frame_nr; i++) {
        struct tpacket3_hdr *frame =
            (struct tpacket3_hdr *)((uint8_t *)ring->rd[0].iov_base +
                                    (i * ring->req.tp_frame_size));
    ...
    for (;;) {
      if (sendto(psk, buffer, buffer_len, 0, (struct sockaddr *)&sa,
